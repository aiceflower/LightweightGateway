...
/* ****************************************************** */
static int transop_deinit_wbsm4(n2n_trans_op_t *arg) {
 ...
 *         |<---- encrypted ---->|
 */
static int transop_encode_wbsm4(n2n_trans_op_t * arg,
			       uint8_t * outbuf,
			       size_t out_len,
			       const uint8_t * inbuf,
			       size_t in_len,
...
static int transop_decode_wbsm4(n2n_trans_op_t * arg,
			       uint8_t * outbuf,
			       size_t out_len,
			       const uint8_t * inbuf,
			       size_t in_len,
...
static int setup_wbsm4_key(transop_speck_t *priv, const uint8_t *key, ssize_t key_size) {
...
static void transop_tick_wbsm4(n2n_trans_op_t * arg, time_t now) { ; }

/* ****************************************************** */
/* Speck initialization function */
int n2n_transop_wbsm4_init(const n2n_edge_conf_t *conf, n2n_trans_op_t *ttt) {
  transop_speck_t *priv;
  const u_char *encrypt_key = (const u_char *)conf->encrypt_key;
  size_t encrypt_key_len = strlen(conf->encrypt_key);

  memset(ttt, 0, sizeof(*ttt));
  ttt->transform_id = N2N_TRANSFORM_ID_WBSM4;

  ttt->tick = transop_tick_wbsm4;
  ttt->deinit = transop_deinit_wbsm4;
  ttt->fwd = transop_encode_wbsm4;
  ttt->rev = transop_decode_wbsm4;
  
  priv = (transop_speck_t*) calloc (1, sizeof(transop_speck_t));
  
  if(!priv) {
    traceEvent(TRACE_ERROR, "cannot allocate transop_wbsm4_t memory");
    return(-1);
  }
  ttt->priv = priv;

  /* Setup the cipher and key */
  return(setup_wbsm4_key(priv, encrypt_key, encrypt_key_len));
}
