...
const char* transop_str(enum n2n_transform tr) {
  ...
  case N2N_TRANSFORM_ID_WBSM4:     return("WBSM4");
...
n2n_edge_t* edge_init(const tuntap_dev *dev, const n2n_edge_conf_t *conf, int *rv) {
  ...
  case N2N_TRANSFORM_ID_TWOFISH:
    ...
    break;
  case N2N_TRANSFORM_ID_WBSM4:
    traceEvent(TRACE_NORMAL, "wbsm4 init start...");
    rc = n2n_transop_wbsm4_init(&eee->conf, &eee->transop);
    traceEvent(TRACE_NORMAL, "wbsm4 init success");
    break;        
#ifdef N2N_HAVE_AES
  ...
  traceEvent(TRACE_NORMAL, "edge init success.");
  //edge_init_success:
  *rv = 0;
  return(eee);
