#ifndef OPEN_WBCRYPTO_AES_H
#define OPEN_WBCRYPTO_AES_H

#include "openwbcrypto/conf.h"

// uint8_t y[4] -> uint32_t x
#define LOAD32H(x, y) \
  do { (x) = ((uint32_t)((y)[0] & 0xff)<<24) | ((uint32_t)((y)[1] & 0xff)<<16) | \
             ((uint32_t)((y)[2] & 0xff)<<8)  | ((uint32_t)((y)[3] & 0xff));} while(0)

// uint32_t x -> uint8_t y[4]
#define STORE32H(x, y) \
  do { (y)[0] = (uint8_t)(((x)>>24) & 0xff); (y)[1] = (uint8_t)(((x)>>16) & 0xff);   \
       (y)[2] = (uint8_t)(((x)>>8) & 0xff); (y)[3] = (uint8_t)((x) & 0xff); } while(0)

// Extract the n byte starting from the low bit of uint32_t
#define BYTE(x, n) (((x) >> (8 * (n))) & 0xff)

/* used for keyExpansion */
// Byte replacement, rotate 1 bit to the left
#define MIX(x) (((AES_SBox[BYTE(x, 2)] << 24) & 0xff000000) ^ ((AES_SBox[BYTE(x, 1)] << 16) & 0xff0000) ^ \
                ((AES_SBox[BYTE(x, 0)] << 8) & 0xff00) ^ (AES_SBox[BYTE(x, 3)] & 0xff))

// uint32_t rotate left by n bits
#define ROF32(x, n)  (((x) << (n)) | ((x) >> (32-(n))))
// uint32_t rotate right by n bits
#define ROR32(x, n)  (((x) >> (n)) | ((x) << (32-(n))))

/* for 128-bit blocks, Rijndael never uses more than 10 rcon values */
// AES-128 round constant
static const uint32_t rcon[10] = {
        0x01000000UL, 0x02000000UL, 0x04000000UL, 0x08000000UL, 0x10000000UL,
        0x20000000UL, 0x40000000UL, 0x80000000UL, 0x1B000000UL, 0x36000000UL
};

static const unsigned char AES_SBox[256] = {
        
};

static const unsigned char AES_inv_SBox[256] = {
        
};

#ifdef __cplusplus
extern "C" {
#endif

    struct aes_context{
        uint32_t eK[44], dK[44];    // encKey, decKey
        int Nr;                     // 10 rounds
    };

    typedef struct aes_context WBCRYPTO_aes_context;

    /******************************************basic function**********************************************/
    WBCRYPTO_aes_context *WBCRYPTO_aes_context_init();
    int WBCRYPTO_aes_init_key(WBCRYPTO_aes_context *ctx, const uint8_t *key, size_t keylen);
    int WBCRYPTO_aes_file_to_key(WBCRYPTO_aes_context *ctx, char *fpath);
    int WBCRYPTO_aes_key_to_file(const WBCRYPTO_aes_context *ctx, char *fpath);
    void WBCRYPTO_aes_context_free(WBCRYPTO_aes_context *ctx);
    int WBCRYPTO_aes_encrypt(const unsigned char *input, unsigned char *output, WBCRYPTO_aes_context *ctx);
    int WBCRYPTO_aes_decrypt(const unsigned char *input, unsigned char *output, WBCRYPTO_aes_context *ctx);

    /********************************************CBC mode************************************************/
    int WBCRYPTO_aes_cbc_encrypt(const unsigned char *in, size_t inlen,
                                  unsigned char *out, size_t outlen,
                                  const WBCRYPTO_aes_context *ctx,
                                  unsigned char ivec[16]);
    int WBCRYPTO_aes_cbc_decrypt(const unsigned char *in, size_t inlen,
                                  unsigned char *out, size_t outlen,
                                  const WBCRYPTO_aes_context *ctx,
                                  unsigned char ivec[16]);

    /********************************************GCM mode************************************************/
    WBCRYPTO_gcm_context *WBCRYPTO_aes_gcm_init(WBCRYPTO_aes_context *key);

    /******************************************gcmfile mode*********************************************/
    WBCRYPTO_gcmfile_context *WBCRYPTO_aes_gcmfile_init(WBCRYPTO_aes_context *key);


#ifdef __cplusplus
}
#endif

#endif //OPEN_WBCRYPTO_AES_H
